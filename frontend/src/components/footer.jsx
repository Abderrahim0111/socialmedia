...
const Footer = ({toggleTheme, settoggleTheme}) => {
  ...
  return (
    <footer className={`${toggleTheme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}  sm:top-0 z-10 sm:w-fit sm:flex sm:flex-col sm:items-center lg:items-start sm:justify-between fixed bottom-0 w-full  p-[0.3rem] sm:p-3 border-t-2 sm:border-t-0 sm:border-r-2 border-[#262626]`}>
      <ul className=" flex items-center lg:items-start justify-evenly sm:justify-start sm:gap-4 sm:flex-col  sm:h-[70%]">
        <Link
          className={` ${toggleTheme === 'dark'? 'lg:hover:bg-[#262626]': 'lg:hover:bg-[#F1F2F5]'} transition-all duration-300 rounded-lg py-1 px-3 lg:w-full cursor-pointer`}
          to="/"
        >
          <li
        ...
        <li
          ...
          className={` hidden sm:flex items-center gap-3  ${toggleTheme === 'dark'? 'lg:hover:bg-[#262626]': 'lg:hover:bg-[#F1F2F5]'} transition-all duration-300 rounded-lg py-1 px-3 lg:w-full cursor-pointer`}
        >
        ...
        {toggleSearchBigScreen && (
          <div className={` hidden sm:block p-3 absolute top-0 bottom-0 left-[4.8rem] ${toggleTheme === 'dark'? 'bg-black': 'bg-white'} border-r-2 border-[#262626] w-80`}>
            <h1 className="hidden lg:block  text-2xl mb-8">Search</h1>
            <li className={` flex items-center gap-2 ${toggleTheme === 'dark'? 'bg-[#262626]': 'bg-[#F1F2F5]'} p-2 rounded-lg mb-6`}>
              {!toggleSearch && (
              ...
              {users.map((user, index) => {
                return (
                  <Link
                    to={user.username}
                    key={index}
                    className={` flex cursor-pointer items-center gap-3 ${toggleTheme === 'dark'? 'bg-[#363636]': 'bg-[#F1F2F5]'} mb-2 p-2 rounded-lg `}
                  >
                    <div className=" border border-[#262626] rounded-full h-9 w-9 overflow-hidden">
                      <img src={user.profileimage} className=" h-full w-full object-contain" alt="" />
                    </div>
                    <p>{user.username}</p>
                  </Link>
                );
        ...
        <li
          className={`flex items-center gap-3  ${toggleTheme === 'dark'? 'lg:hover:bg-[#262626]': 'lg:hover:bg-[#F1F2F5]'} transition-all duration-300 rounded-lg py-1 px-3 lg:w-full cursor-pointer`}
          onClick={() => {
            settoggleSearchBigScreen(false);
            setshowModal(true);
            setisMore(false);
        ...
        <Link
          className={`hidden relative sm:block  ${toggleTheme === 'dark'? 'lg:hover:bg-[#262626]': 'lg:hover:bg-[#F1F2F5]'} transition-all duration-300 rounded-lg py-1 px-3 w-full cursor-pointer`}
          to="/notifications"
        >
          <li
            className="flex items-center gap-3"
          ...
          <div className={` bg-red-500 ${toggleTheme === 'dark'? '': 'text-white'} rounded-full h-4 w-4 text-sm top-0 left-0 absolute  flex items-center justify-center`}>
      ...
      <div
        ...
        className={`hidden relative sm:flex items-center gap-3  ${toggleTheme === 'dark'? 'lg:hover:bg-[#262626]': 'lg:hover:bg-[#F1F2F5]'} transition-all duration-300 rounded-lg py-1 px-3 w-full cursor-pointer`}
      >
      ...
      {isMore && (
        <div className={`absolute left-[60px] w-max bottom-14 p-3 ${toggleTheme === 'dark'? 'bg-[#262626]': 'bg-[#F1F2F5]'} rounded-xl`}>
          <ul className=" ">
            <Link to="/activity">
            <li
              onClick={() => {
